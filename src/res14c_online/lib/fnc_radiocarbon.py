...
def calibrate(age, uncert, curve_conv_age, curve_uncert, normalize = False):
	...
	dist = (np.exp(-(age - curve_conv_age)**2 / (2 * sigma_sum)) / np.sqrt(sigma_sum))
	if normalize:
		s = dist.sum()
		if s > 0:
			dist /= s
...
def calc_range(curve_cal_age, distribution, p = 0.9545, p_threshold = 0.0001, max_multiplier = 32):
	...
	v_min, v_max = curve_cal_age.min(), curve_cal_age.max()
	p_diff_opt = np.inf
	v_opt = [v_min, v_max]
	mul = 2
	while True:
		v_min, v_max, p_diff = _find_rng(curve_cal_age, distribution, v_min, v_max, mul)
